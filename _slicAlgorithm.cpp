...
void SlicImpl::Metadata(int &spCount)  {
  if (spCount)
  {
    if (seedsX.size() != spCount ||
        seedsY.size() != spCount ||
        seedsC[0].size() != spCount ||
        seedsC[1].size() != spCount ||
        seedsC[2].size() != spCount)
    {
      cout << "fatal error : number of superpixeles is irregular.";
      exit(1);
    }
    return;
  }
  
  spCount = (int)seedsX.size();
  string filename = "../data/__metadata.csv";
  
  
  if (seedsY.size() != spCount ||
      seedsC[0].size() != spCount ||
      seedsC[1].size() != spCount ||
      seedsC[2].size() != spCount)
  {
...
int SlicImpl::saveCentroids( int label, int spCount)
{
  ofstream xfile;
  ofstream yfile;
  ofstream color1file;
  ...
  if (!spCount){//first time is zero, create new files
    ofstream labelsfile("../data/_labels.csv", ofstream::out );
    labelsfile << label << "," << endl;
    labelsfile.close();

    xfile.open("../data/_x.csv", ofstream::out);
    yfile.open("../data/_y.csv", ofstream::out );
    color1file.open("../data/_color1.csv", ofstream::out );
    color2file.open("../data/_color2.csv", ofstream::out );
    color3file.open("../data/_color3.csv", ofstream::out );
  } else {
    ofstream labelsfile("../data/_labels.csv", ofstream::out | ofstream::app);
    labelsfile << label << "," << endl;
    labelsfile.close();

    xfile.open("../data/_x.csv", ofstream::out | ofstream::app);
    yfile.open("../data/_y.csv", ofstream::out | ofstream::app);
    color1file.open("../data/_color1.csv", ofstream::out | ofstream::app);
    color2file.open("../data/_color2.csv", ofstream::out | ofstream::app);
    color3file.open("../data/_color3.csv", ofstream::out | ofstream::app);
